...
export default class NotificationWindowHelpers {

  static showPositionedNotificationWindow(wnd, mainWindow, zoomLevel, notifyPosition) {
    // NB: Each zoom level corresponds to a 20% change
    let scaleFactor = 1 + (zoomLevel * 0.2);
    ...
    let coords = NotificationWindowHelpers.calculateHostCoordinates(options);
  ...
  static calculateHostCoordinates(options) {
    ...
    let display = NotificationWindowHelpers.getDisplayForHost(parent, screenPosition, screenApi);
    ...
    let targetHeight = (maxCount + 1) * size.height;
