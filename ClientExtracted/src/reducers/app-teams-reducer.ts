...
export function reduce(state: AppTeamsState = initialState, action: Action & {selectTeam: boolean}) {
  switch (action.type) {
...
function handleNewTeams(state: AppTeamsState, newTeams: Array<Team>, selectTeam: boolean = true) {
  ...
  const hiddenTeams = state.hiddenTeams.filter((team) => !teamIds.includes(team));
...
function handleHideTeam(state: AppTeamsState, teamId: string) {
