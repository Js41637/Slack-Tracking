...
export default class DownloadListener extends ReduxComponent {
  ...
  initializeDownloadsDirectory(platform) {
    ...
    switch (platform) {
    case 'win32':
      downloadDir = getKnownFolder('Downloads') || p`${'HOME'}/Downloads`;
      break;
    case 'darwin':
  ...
  updateMainWindowProgress() {
    ...
    if (_.find(this.downloadItems)) {
      let itemProgress = _.map(this.downloadItems, (item) => {
        return this.getDownloadItemProgress(item);
      });
      maxProgress = _.max(itemProgress);
